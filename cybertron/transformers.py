    ...
    def forward(
        ...
        if utils.exists(self.rotary_pos_emb):
            max_rotary_emb_length = max(
                list(
                    map(
                        lambda m: (m.shape[1] if utils.exists(m) else 0) + x.shape[1],
                        mems,
                    )
                )
            )
...
class ContinuousTransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.emb_dropout = nn.Dropout(emb_dropout)
        self.project_in = (
            nn.Linear(dim_in, dim) if utils.exists(dim_in) else nn.Identity()
        )
        self.attn_layers = attn_layers
        self.norm = nn.LayerNorm(dim)
        self.project_out = (
            nn.Linear(dim, dim_out) if utils.exists(dim_out) else nn.Identity()
        )
...
class Cybertron(nn.Module):
    def __init__(self, *, dim, tie_token_emb=False, **kwargs):
        ...
        enc_transformer_kwargs = utils.pick_and_pop(
        ...
        dec_transformer_kwargs = utils.pick_and_pop(
